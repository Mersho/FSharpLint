...
let private runner (args:AstNodeRuleParams) =
    let maybeSuggestedFix = 
        match args.SyntaxArray.[args.SyntaxArray.[args.NodeIndex].ParentIndex].Actual with
        | AstNode.ModuleDeclaration(SynModuleDecl.Let(_, _, range)) ->
            Some({ FromRange = range; FromText = "let"; ToText = "" })
        | AstNode.Expression(SynExpr.LetOrUse(_, false, _, _, range)) -> 
