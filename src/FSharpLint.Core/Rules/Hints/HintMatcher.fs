...
let private hintError (config: HintErrorConfig) =
    //let matched = FormatHint.toString false None config.Args config.MatchedVariables None config.Hint.MatchedNode
    ...
    | Suggestion.Expr(expr) ->
        ...
        // let toText = FormatHint.toString true config.ParentAstNode config.Args config.MatchedVariables None (HintExpr expr)
